                            ...
                            <label class="block text-base font-semibold text-gray-700 dark:text-gray-300 mb-3">
                                <span class="flex items-center">
                                    <svg class="w-5 h-5 mr-2 text-amber-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8c-1.657 0-3 .895-3 2s1.343 2 3 2 3 .895 3 2-1.343 2-3 2m0-10v.01M12 6v.01"/>
                                    </svg>
                                    Budget Maximum (Ar)
                                    <span class="text-red-500 ml-1">*</span>
                                </span>
                                ...
                                <label class="flex items-center space-x-3 cursor-pointer group">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                            Si activée, cette catégorie sera disponible pour les nouvelles transactions et apparaîtra dans les listes actives.
                                        </p>
                                    </div>
                                </label>
                            </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div class="space-y-5">
                            <div class="flex items-start bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl">
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Code comptable</dt>
                                    <dd class="text-xl text-gray-900 dark:text-white font-mono font-bold mt-1">{{ $detail['code_comptable'] }}</dd>
                                </div>
                            </div>
                            ...
                            <div class="flex items-start bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl">
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Nom de la catégorie</dt>
                            ...
                            <div class="flex items-start bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl">
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Budget Maximum</dt>
                            ...
                            <div class="flex items-start bg-gray-50 dark:bg-gray-700/50 p-4 rounded-xl">
                                ...
                                <div>
                                    <dt class="text-sm font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wide">Description</dt>
                                    <dd class="text-lg text-gray-900 dark:text-white mt-1 leading-relaxed">{{ $detail['description'] }}</dd>
                                </div>
                            </div>
                            @endif
                        </div>
                    ...
                    <div class="space-y-6">
                        ...
                        <div>
                            ...
                            <div class="space-y-2">
                                ...
                                <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-gray-700 rounded-lg hover:shadow-md transition-all">
                                    <div>
...
{{-- Modal création transaction rapide --}}
@if($showNewTransactionModal)
<div class="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-xl w-full max-w-lg transform transition-all duration-300">
        <div class="p-6">
            <div class="flex justify-between items-start mb-6">
                <h3 class="text-lg font-bold text-gray-900 dark:text-white">Nouvelle Transaction</h3>
                <button wire:click="closeNewTransactionModal" class="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300 transition-colors">
            ...
            <form wire:submit.prevent="saveTransaction" class="space-y-4">
                ...
                <div>
                    ...
                    <input type="text" wire:model.defer="newTransaction.description"
                ...
                <div>
                    ...
                    <input type="number" wire:model.defer="newTransaction.montant"
                           step="0.01" min="0" placeholder="0"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    @error('newTransaction.montant') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>

                <!-- Date -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Date</label>
                    <input type="date" wire:model.defer="newTransaction.date_transaction"
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    @error('newTransaction.date_transaction') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
...
<div>
    ...
    <input type="text" 
           wire:model.defer="newTransaction.partenaire_nom"
           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"
           placeholder="Entrer le nom du partenaire">
    @error('newTransaction.partenaire_id') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
</div>

                <!-- Justificatif -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Justificatif</label>
                    <input type="text" wire:model.defer="newTransaction.justificatif"
                           placeholder="Numéro de facture, reçu..."
                           class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white">
                    @error('newTransaction.justificatif') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>

                <!-- Notes -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">Notes</label>
                    <textarea wire:model.defer="newTransaction.notes" rows="3"
                              placeholder="Notes complémentaires..."
                              class="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 bg-white dark:bg-gray-700 text-gray-900 dark:text-white"></textarea>
                    @error('newTransaction.notes') <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                </div>
    ...
    <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
        ...
        <div class="inline-block w-full max-w-2xl my-8 overflow-hidden text-left align-middle transition-all transform bg-white dark:bg-gray-800 shadow-2xl rounded-xl">
            ...
            <div class="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-gray-700 bg-gradient-to-r from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800">
                <div>
                    <h3 class="text-lg font-bold text-gray-900 dark:text-white">{{ $transactionDetails['reference'] ?? 'Détail Transaction' }}</h3>
                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ $transactionDetails['date'] ?? '' }}</p>
                </div>
            ...
            <div class="px-6 py-6 space-y-4">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $transactionDetails['description'] ?? '' }}</p>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <span class="mt-1 inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                            {{ ($transactionDetails['statut'] ?? '') === 'entrer' ? 'bg-green-100 text-green-800 dark:bg-green-900 dark:text-green-200' : 'bg-red-100 text-red-800 dark:bg-red-900 dark:text-red-200' }}">
                            {{ ucfirst($transactionDetails['statut'] ?? '') }}
                        </span>
                    </div>
                    ...
                    <div>
                        ...
                        <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $transactionDetails['partenaire'] }}</p>
                    </div>
                    @endif
                </div>

                @if($transactionDetails['justificatif'] ?? false)
                <div>
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Justificatif</label>
                    <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $transactionDetails['justificatif'] }}</p>
                </div>
                @endif
                ...
                <div>
                    ...
                    <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $transactionDetails['notes'] }}</p>
                </div>
                @endif

                <div>
                    <label class="block text-sm font-medium text-gray-500 dark:text-gray-400">Créé le</label>
                    <p class="mt-1 text-sm text-gray-900 dark:text-white">{{ $transactionDetails['created_at'] ?? '' }}</p>
                </div>
            </div>

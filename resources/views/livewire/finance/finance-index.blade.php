{{-- resources/views/livewire/finance/finance-index.blade.php - CORRIGÉE --}}
<div class="space-y-6">
    <!-- Header -->
    <div class="flex justify-between items-center">
        ...
        <div class="flex space-x-2">
            @if($activeTab === 'transactions')
                <button wire:click="createTransaction" class="inline-flex items-center px-4 py-2 bg-green-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-green-700">
                    ➕ Nouvelle Transaction
                </button>
            @elseif($activeTab === 'comptes')
                <button wire:click="createCompte" class="inline-flex items-center px-4 py-2 bg-blue-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-700">
                    🏦 Nouveau Compte
                </button>
            @endif
        </div>
        ...
        <div class="border-b border-gray-200">
            <nav class="-mb-px flex space-x-8 px-6">
                ...
                <button 
                    ...
                    class="py-4 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'comptes' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    🏦 Comptes ({{ $comptes->count() }})
                </button>
                <button 
                    wire:click="setActiveTab('rapports')"
                    class="py-4 px-1 border-b-2 font-medium text-sm {{ $activeTab === 'rapports' ? 'border-blue-500 text-blue-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300' }}">
                    📈 Rapports
                </button>
            </nav>
        </div>
    ...
    <!-- Modals - ✅ SUPPRIMÉ $users CAR ON UTILISE DES CHAMPS LIBRES -->

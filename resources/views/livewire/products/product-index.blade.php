    ...
    <div class="space-y-6 max-w-7xl mx-auto">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <p class="text-sm text-gray-600 dark:text-gray-400">Gérez vos produits de transport</p>
            </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                    <thead class="bg-gray-50 dark:bg-gray-700">
                        <tr>
                            <th wire:click="sortBy('nom')" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400">
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden sm:table-cell dark:text-gray-400">Variété</th>
                            <th wire:click="sortBy('qte_variable')" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400">
                                <div class="flex items-center gap-1">
                                    <span>📦 Stock</span>
                            ...
                            <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider hidden md:table-cell dark:text-gray-400">Capacité</th>
                            <th wire:click="sortBy('prix_reference_mga')" class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100 dark:hover:bg-gray-600 dark:text-gray-400 hidden lg:table-cell">
                                <div class="flex items-center gap-1">
                                    <span>Prix réf.</span>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200 dark:bg-gray-800 dark:divide-gray-700">
                        @forelse($produits as $produit)
                            @php
                                ...
                                if ($stockActuel <= 0) {
                                    $stockStatus = 'Vide';
                                    $stockColor = 'text-red-700 dark:text-red-400';
                                    $stockBgColor = 'bg-red-50 dark:bg-red-900/30';
                                    ...
                                    $stockBgColor = 'bg-orange-50 dark:bg-orange-900/30';
                                } elseif ($pourcentageStock <= 80) {
                                    $stockStatus = 'Correct';
                                    $stockColor = 'text-blue-600 dark:text-blue-400';
                                    $stockBgColor = 'bg-blue-50 dark:bg-blue-900/30';
                                } else {
                                    $stockStatus = 'Plein';
                                    $stockColor = 'text-green-600 dark:text-green-400';
                                    $stockBgColor = 'bg-green-50 dark:bg-green-900/30';
                                }
                            @endphp
                            
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700 {{ $stockActuel <= 0 ? 'bg-red-25 dark:bg-red-900/10' : '' }}">
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div>
                                        <div class="text-sm font-medium text-gray-900 dark:text-white flex items-center">
                                            {{ $produit->nom }}
                                            @if($stockActuel <= 0)
                                                <span class="ml-2 inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400">
                                                    ⚠️ Stock vide
                                                </span>
                                            @elseif($pourcentageStock <= 20)
                                        ...
                                        @if($produit->description)
                                            <div class="text-xs text-gray-500 dark:text-gray-400 sm:hidden mt-1">{{ Str::limit($produit->description, 30) }}</div>
                                        @endif
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex flex-col">
                                        ...
                                        <div class="w-full bg-gray-200 rounded-full h-2 dark:bg-gray-700">
                                            <div class="h-2 rounded-full {{ $stockActuel <= 0 ? 'bg-gray-300 dark:bg-gray-600' : ($pourcentageStock <= 20 ? 'bg-red-500' : ($pourcentageStock <= 50 ? 'bg-orange-500' : ($pourcentageStock <= 80 ? 'bg-blue-500' : 'bg-green-500'))) }}" 
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-sm text-gray-900 dark:text-white hidden lg:table-cell">
                                    @if($produit->prix_reference_mga)
                                        <div class="flex flex-col">
                                            ...
                                            <span class="text-gray-500 text-xs dark:text-gray-400">MGA/{{ $produit->unite }}</span>
                                        </div>
                                    @else
                                ...
                                <td class="px-4 py-4 whitespace-nowrap">
                                    <div class="flex flex-col gap-1">
                                        <button wire:click="toggleActif({{ $produit->id }})" class="inline-flex px-2 py-1 text-xs font-semibold rounded-full 
                                            {{ $produit->actif ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400' : 'bg-red-100 text-red-800 dark:bg-red-900/30 dark:text-red-400' }}">
                                ...
                                <td class="px-4 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        <button wire:click="edit({{ $produit->id }})" 
                                                class="text-blue-600 hover:text-blue-900 p-1 rounded-full hover:bg-blue-100 dark:text-blue-400 dark:hover:text-blue-300 dark:hover:bg-blue-900/30" 
                                                title="Modifier">
                                        ...
                                        <button 
                                            ...
                                            class="text-red-600 hover:text-red-900 p-1 rounded-full hover:bg-red-100 dark:text-red-400 dark:hover:text-red-300 dark:hover:bg-red-900/30" 
                                            title="Supprimer">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="mt-4 bg-white rounded-lg shadow p-4 dark:bg-gray-800">
            <h4 class="text-sm font-medium text-gray-900 dark:text-white mb-3">📊 Légende des niveaux de stock</h4>
            <div class="grid grid-cols-2 md:grid-cols-5 gap-3 text-xs">
                <div class="flex items-center">
                    <div class="w-3 h-3 bg-gray-300 rounded-full mr-2 dark:bg-gray-600"></div>
                    <span class="text-gray-600 dark:text-gray-400">🔴 Vide (0%)</span>
                </div>
                <div class="flex items-center">
                    <div class="w-3 h-3 bg-red-500 rounded-full mr-2"></div>
                    <span class="text-red-600 dark:text-red-400">🟠 Critique (≤20%)</span>
                </div>
                <div class="flex items-center">
                    <div class="w-3 h-3 bg-orange-500 rounded-full mr-2"></div>
                    <span class="text-orange-600 dark:text-orange-400">🟡 Faible (≤50%)</span>
                </div>
                <div class="flex items-center">
                    <div class="w-3 h-3 bg-blue-500 rounded-full mr-2"></div>
                    <span class="text-blue-600 dark:text-blue-400">🔵 Correct (≤80%)</span>
                </div>
                <div class="flex items-center">
                    <div class="w-3 h-3 bg-green-500 rounded-full mr-2"></div>
                    <span class="text-green-600 dark:text-green-400">🟢 Plein (>80%)</span>
                </div>
            </div>
        </div>
                        ...
                        <form wire:submit="save">
                            <div class="bg-white px-4 pt-5 pb-4 sm:p-6 sm:pb-4 dark:bg-gray-800">
                                ...
                                <div class="space-y-4">
                                    ...
                                    <!-- Variété -->
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Variété</label>
                                        <input 
                                            wire:model="variete"
                                            type="text"
                                            class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                        ...
                                        >
                                        @error('variete') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    ...
                                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            ...
                                            <select wire:model="unite" class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500">
                                        ...
                                        <!-- Poids moyen -->
                                        <div>
                                            <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Poids/sac (kg) *</label>
                                            <input 
                                                ...
                                                class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm py-2 px-3 focus:ring-blue-500 focus:border-blue-500 text-sm dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:placeholder-gray-400 dark:focus:ring-blue-500 dark:focus:border-blue-500"
                                            >
                                            @error('poids_moyen_sac_kg_max') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                        </div>
                                    </div>

                                    <!-- Prix de référence -->
                                    <div>
                                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Prix de référence (MGA)</label>
                                        <input 
                                            ...
                                            placeholder="3600"
                                        >
                                        @error('prix_reference_mga') <p class="mt-1 text-sm text-red-600 dark:text-red-400">{{ $message }}</p> @enderror
                                    </div>
                                    ...
                                    <div class="flex items-center">
                                        ...
                                        >
                                        <label class="ml-2 block text-sm text-gray-900 dark:text-gray-300">Produit actif</label>
                                    </div>
                                </div>
                            </div>
